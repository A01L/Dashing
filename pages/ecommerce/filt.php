...
@media (max-width: 991.98px) {
  ...
  followCursor() // вызываем функцию followCursor
		...
		<li>
			<a href="../apps/contact/plist_a.php">
				<i class="icofont-ui-user"></i>
				 <span class="link-title">Пероснал</span>
			</a>
		</li>
		...
		<li>
			...
			<ul class="nav sub-menu">
				<li>
					<a href="../apps/contact/notes.php">Все</a>
				</li>
...
<div class="col-12" style="float: left;">
	<div class="card mb-30">
		...
		<form style="margin-top: 20px;" method="post" action="#">
			<input type="date" name="date" style="background-color: #6350E7; color: white; border: 2px solid #6350E7; border-radius: 10px 0px 0px 10px; padding-left: 10px;">
			<button style="background-color: #1A2334; border: 2px solid #6350E7; color: #6350E7; border-radius: 0px 10px 10px 0px;">Получить отчет</button>
			<br><br>
			
			<?php 
				if (!$_POST['date']) {
					echo "<h3>Выбирайте дату</h3>";
				}
				else {
		...
		<table class="style--four table-striped text-nowrap">
            ...
            if($result = $conn->query($sql)){
                ...
                foreach($result as $row){
                    ...
                    // check new message
                	$datefb = $row["date"];
                	$dateo = mb_eregi_replace("(.*)[^.]{6}$", '\\1', $datefb);
                	if ($row['type'] == "minus") {
                		$symb = "-";
                		$color = "#DE5A5E";
                	}
				...
				<tr>
					...
					<td style="color: <?php echo $clcm; ?>; width: 400px;"><p style="width: 400px;"><textarea style="width: 400px; height: 100px;" class="theme-input-style"><?php echo $comtd; ?></textarea></p></td>

					<td style="color: <?php echo $color; ?>; width: 200px;"><?php echo $symb; ?><?php echo $row["sum"]; ?></td>
					<td><?php echo get_data_db($connect, "personals", "name", "id", $row['personal']); ?></td>
				</tr>
		...
		<table class="style--four table-striped text-nowrap">
            ...
            if($result = $conn->query($sql)){
				...
				<tr>
					...
					<td style="width: 100px;"><?php echo $row["date"]; ?></td>
					<td style="width: 180px;"><?php echo get_data_db($connect, "personals", "name", "id", $row["personal"]); ?></td>
					<td style="color: #50C878;"><?php echo "+ ".$row['total']; ?></td>

					<td style="color: #DE5A5E;"><?php echo "- ".$row['minus'];; ?></td>
					<td style="color: #50C878;"><?php echo "+ ".$row['plus'];; ?></td>
